...
export default function RentPaymentPage({ data }) {
  ...
  // Base rows from server state
  const base = useMemo(() => {
    const arr = (data?.rentPayments || data?.rent_payments || []).slice();
    arr.sort((a, b) => {
      const ta = new Date(a?.date || a?.month || 0).getTime() || 0;
      const tb = new Date(b?.date || b?.month || 0).getTime() || 0;
      return tb - ta;
    });
    return arr;
  }, [data]);
  ...
  useEffect(() => {
    const onResize = () => {
      ...
      setView(v => {
        if (mobile && v === 'table') return 'cards';
        if (!mobile && v === 'cards') return 'table';
        return v;
      });
    };
  ...
  const rows = useMemo(() => {
    return base.map(r => {
      const dateRaw = r?.date || r?.month || '';
      const ymd = normalizeYMD(dateRaw);
      const roomStr = Array.isArray(r?.room) ? r.room.join(', ') : (r?.room ?? '');
      const ts = new Date(dateRaw || 0).getTime() || 0;
      return {
        ...r,
        _date: dateRaw,
        _ymd: ymd,
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter(r => {
      // date range
      if (from && (r._ymd || '') < from) return false;
      if (to && (r._ymd || '') > to) return false;
      // mode
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
        ...
        <div style={{ marginLeft: "auto", fontWeight: 900, color: COLORS.deep }}>Total: {fmtMoney(totalAmount)}</div>
      </div>
                ...
                <tbody>
                  {pageRows.map((r, idx) => (
                    <tr key={idx} style={{ borderBottom: "1px solid #eee" }}>
                      <td style={{ padding: 8 }}>{r._date || r._ymd}</td>
                      <td style={{ padding: 8 }}>{r?.name || '-'}</td>
                      <td style={{ padding: 8 }}>{r?._roomStr || '-'}</td>
                      <td style={{ padding: 8 }}>{r?.days ?? '-'}</td>
                      <td style={{ padding: 8 }}>{fmtMoney(r?.amount)}</td>
              ...
              {pageRows.map((r, idx) => (
                <div key={idx} className="card" style={{ border: `1px solid ${COLORS.border}`, borderRadius: 10, padding: 10 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'baseline' }}>
                    <div style={{ fontWeight: 800, color: COLORS.deep, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {r?.name || '-'}
                    </div>
                    <div style={{ fontSize: 12, color: COLORS.muted }}>{r._date || r._ymd}</div>
                  </div>
                  <div style={{ fontSize: 12, color: COLORS.muted }}>
                    Room {r?._roomStr || '-'} • {r?.mode || '-'} • {r?.days ?? '-'} day(s)
                  </div>
                  <div style={{ marginTop: 6, fontWeight: 900, color: COLORS.deep }}>{fmtMoney(r?.amount)}</div>
                </div>
              ))}
            </div>
